        ...
        <form action="{{ route('checkout.process') }}" method="POST" class="contents">
            ...
            <div class="lg:col-span-2">
                ...
                <div class="bg-white rounded-lg shadow p-6 mb-6">
                    <h2 class="text-lg font-semibold mb-4">{{ __('storefront.checkout.customer_info') }}</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('storefront.checkout.name') }}</label>
                            <p class="text-gray-900">{{ $customer->name }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('storefront.checkout.email') }}</label>
                    ...
                    <a href="{{ route('customer.profile') }}" class="text-primary-600 text-sm hover:underline mt-2 inline-block">{{ __('storefront.checkout.update_profile') }}</a>
                </div>

                <!-- Order Items -->
                <div class="bg-white rounded-lg shadow p-6 mb-6">
                    <h2 class="text-lg font-semibold mb-4">{{ __('storefront.checkout.order_items') }}</h2>
                    <div class="space-y-4">
                            ...
                            <div class="flex items-center justify-between py-3 border-b">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500">{{ $item->start_date->format('d M Y') }} - {{ $item->end_date->format('d M Y') }} ({{ $item->days }} {{ __('storefront.days') }})</p>
                                    </div>
                                </div>
                ...
                <!-- Notes -->
                <div class="bg-white rounded-lg shadow p-6 mb-6">
                    <h2 class="text-lg font-semibold mb-4">{{ __('storefront.checkout.additional_notes') }}</h2>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    <label class="flex items-start">
                        ...
                        <span class="text-sm text-gray-600">
                            {{ __('storefront.checkout.agree_terms_text') }} <a href="#" class="text-primary-600 hover:underline">{{ __('storefront.checkout.terms_and_conditions') }}</a> {{ __('storefront.checkout.deposit_notice') }}
                        </span>
                    </label>
                </div>
            </div>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg shadow p-6 sticky top-24">
                    <h2 class="text-lg font-semibold mb-4">{{ __('storefront.checkout.order_summary') }}</h2>

                    <div class="mb-6 border-b pb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('storefront.checkout.discount_code') }}</label>
                        <div class="flex gap-2">
                            <input type="text" id="discount_code" value="{{ session('checkout_discount_code') }}" class="w-full border rounded-lg px-3 py-2 text-sm uppercase" placeholder="{{ __('storefront.checkout.enter_code') }}">
                            <button type="button" id="apply_discount" class="bg-gray-800 text-white px-4 py-2 rounded-lg text-sm hover:bg-gray-700 transition">{{ __('storefront.checkout.apply') }}</button>
                        </div>
                        <p id="discount_message" class="text-xs mt-2 {{ session('checkout_discount_amount') ? 'text-green-600' : 'hidden' }}">
                            {{ session('checkout_discount_amount') ? __('storefront.checkout.discount_applied') : '' }}
                        </p>
                    </div>

                    <div class="space-y-3 mb-6">
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('storefront.checkout.subtotal') }}</span>
                            <span>Rp {{ number_format($grossTotal ?? $subtotal, 0, ',', '.') }}</span>
                        </div>

                        @if(isset($categoryDiscountAmount) && $categoryDiscountAmount > 0)
                            <div class="flex justify-between text-green-600">
                                <span>{{ __('storefront.checkout.discount') }} ({{ $categoryName }})</span>
                        ...
                        @if(isset($dailyDiscountAmount) && $dailyDiscountAmount > 0)
                            <div class="flex justify-between text-green-600">
                                <span>{{ $dailyDiscountName ?? __('storefront.checkout.daily_discount') }}</span>
                        ...
                        @if(isset($datePromotionAmount) && $datePromotionAmount > 0)
                            <div class="flex justify-between text-green-600">
                                <span>{{ $datePromotionName ?? __('storefront.checkout.special_promo') }}</span>
                        ...
                        <div id="discount_row" class="flex justify-between text-green-600 {{ isset($discountAmount) && $discountAmount > 0 ? '' : 'hidden' }}">
                            <span>{{ __('storefront.checkout.discount_coupon') }}</span>
                            <span id="discount_amount">-Rp {{ number_format($discountAmount ?? 0, 0, ',', '.') }}</span>
                        </div>

                        @if($deposit > 0)
                        <div class="flex justify-between">
                            <span class="text-gray-600">{{ __('storefront.checkout.deposit') }}</span>
                    ...
                    <div class="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                        <p class="text-xs font-semibold text-green-700 mb-1">{{ __('storefront.checkout.active_promos') }}</p>
                    ...
                    <button type="submit" class="w-full bg-primary-600 text-white py-3 rounded-lg font-semibold hover:bg-primary-700 transition">
                        {{ __('storefront.checkout.confirm_booking') }}
                    </button>

                    <p class="text-xs text-gray-500 mt-4 text-center">
                        {{ __('storefront.checkout.confirm_notice') }}
                    </p>
                </div>
            </div>
        </form>
...
<script>
    document.getElementById('apply_discount').addEventListener('click', function() {
        ...
        .then(data => {
            btn.disabled = false;
            btn.innerHTML = '{{ __('storefront.checkout.apply') }}';
        ...
        .catch(error => {
            ...
            btn.innerHTML = '{{ __('storefront.checkout.apply') }}';
            msg.classList.remove('hidden');
            msg.textContent = '{{ __('storefront.checkout.error_try_again') }}';
            msg.classList.add('text-red-600');
        });
    });
</script>
